package scripts.api.scriptapi.antiban;
...
public final class AntiBan {
	...
	/**
	 * Eats/drinks the item specified if your current HP percent is less than or
	 * equal to the value generated by ABCUtil. Note that if there is any
	 * delay/lag that is longer than 3000 milliseconds between the time the
	 * food/drink was clicked and when your players HP is changed the tracker
	 * will not be reset and you will have to reset it manually.
	 *
	 * @param option
	 *            The option to click the food/drink with (this is normally
	 *            "Eat" or "Drink"). Input an empty string to have the method
	...
	public static boolean eat(String option, RSItem item) {
		...
		final double current_hp = Player.getRSPlayer().getHealthPercent();
			...
			if (!option.isEmpty() && Clicking.click(option, item)) {
				...
				if (Timing.waitCondition(new Condition() {
					...
					public boolean active() {
						...
						return Player.getRSPlayer().getHealthPercent() > eat_at;
					}
				}, 3000)) {
	...
	/**
	 * Walks to the next anticipated resource. Note that you must calculate
	 * which resource is the next anticipated resource yourself. You must also
	 * reset the tracker yourself after the current resource is gone/depleted.
	 * The next anticipated resource generally can be retrieved by calling
	 * #getObject(int) or #getObject(java.lang.String).
	 *
	 * @param anticipated
	 *            The next anticipated resource.
	...
	public static boolean goToAnticipated(Positionable anticipated) {
		if (anticipated != null) {
			sleepReactionTime();
			return Walking.walkPath(Walking.generateStraightPath(anticipated.getPosition()));
		}
		return false;
	}
	...
	private static void debug(Object message) {
		Logger.getLogger().print(Logger.ABC_LOGS, "[ABC2] " + message);
	}

	public static void walkToPosition(Positionable location) {
		WalkingPreference pref = AntiBan.getABCUtil()
				.generateWalkingPreference(Player.getPosition().distanceTo(location));
		if (pref == WalkingPreference.MINIMAP)
			Walking.walkPath(Walking.generateStraightPath(location));
		else
			Walking.walkScreenPath(Walking.generateStraightScreenPath(location));
	}
