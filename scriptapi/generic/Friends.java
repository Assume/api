...
public class Friends {
	...
	/**
	 * Gets all the {@[member="Link"] RSFriend}'s that are in
	 * your friend list.
	 * 
	 * @return Array with all your {@[member="Link"] RSFriend}
	 *         's
	 */
	public static RSFriend[] getAll() {
		final RSInterfaceChild container = Interfaces.get(PARENT_WIDGET,
				FRIEND_CONTAINER);
		...
		if (container != null) {
			...
			if (children != null) {
				for (int i = 0; i < children.length; i++) {
					...
					RSInterfaceComponent statusInterface = children[children[++i]
							.getTextureID() == -1 ? i : ++i];
	 ...
	 * 
	 * @param online
	 *            <tt>true</tt> if you want to get all your online friends,
	...
	/**
	 * Gets the {@[member="Link"] RSFriend} that matches the
	 * specified name Note: Will return <tt>null</tt> if no {@[member="Link"]
	 *  RSFriend} is found. Note: The doesn't have to be case
	 * sensitive
	 * 
	 * @return {@[member="Link"] RSFriend} that matches the
	 *         specified name
	 */
	 ...
	 * 
	 * @param name
	 *            the name of the friend you want to add
	 * @return <tt>true</tt> if the friend is successfully added
	 */
	public static boolean addFriend(String name) {
		if (Friends.getFriend(name) != null)
			return true;
		final RSInterfaceChild addButton = Interfaces.get(PARENT_WIDGET,
				ADD_FRIEND);
		final int count = getAmount();
		return clickFriendButton(addButton, name)
	...
	public static boolean deleteFriend(String name) {
		if (Friends.getFriend(name) == null)
			return true;
		final RSInterfaceChild deleteButton = Interfaces.get(PARENT_WIDGET,
				DELETE_FRIEND);
		final int count = getAmount();
		return clickFriendButton(deleteButton, name)
				&& Timing.waitCondition(new Condition() {
					@Override
					public boolean active() {
						return getAmount() < count;
	 ...
	 * 
	 * @param button
	 *            the interface child to click
	 * @param friendName
	 *            the friend name to type afterwards
	 * @return <tt>true</tt> if the button is successfully clicked
	 */
	private static boolean clickFriendButton(RSInterfaceChild button,
			String friendName) {
		if (button != null && GameTab.open(GameTab.TABS.FRIENDS)
